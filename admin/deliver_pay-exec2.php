...
if($row['AmountPaid'] >= $row['AmountPayable']) {
	...
	$update = $conn->query("UPDATE orders_total SET PaymentStatus='Partially Paid', OrderStatus='Being Prepared', PreparedBy='$_SESSION[userid]'
		WHERE OrdersID='$ordersid'");

}

header('location: deliver_pay-exec3.php?ordersid=' .$ordersid. '');
