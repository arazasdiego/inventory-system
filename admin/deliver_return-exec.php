...
if(isset($_POST['submit'])) {
	...
	if (filter_var($qty, FILTER_VALIDATE_INT) === 1 || !filter_var($qty, FILTER_VALIDATE_INT) === false) {
    	...
    	echo '<script>
			...
			window.location.href="deliver_return.php?id=' .$id. '";
			</script>';
	}
	...
	if($ctr == 3) {
		...
		$update1 = $conn->query("UPDATE orders SET ReturnedQty=ReturnedQty+'$qty', DeliveredQty=DeliveredQty-'$qty'
			WHERE ID='$id'");
