...
$orders = $conn->query("SELECT o.ProductID, o.OrderedQty, o.PickedupQty,

	p.CostPrice, p.Stock, p.CategoryID,
...
while($row = $orders->fetch_assoc()) {
	...
	if($row['OrderedQty'] > $row['Stock']) {
		...
		$po_insert = $conn->query("INSERT INTO auto_po(OrdersID, ProductID, PendingQty, DateAdded, SupplierID, Price, Total)
			VALUES('$ordersid', '$row[ProductID]', '$requestedqty', '$da', 'None', '$row[CostPrice]', '$total')");

			
		$update1 = $conn->query("UPDATE product SET Stock=0
				WHERE ProductID='$row[ProductID]'");

		$update2 = $conn->query("UPDATE orders SET PickedupQty=PickedupQty+'$row[Stock]', PickedupStatus='Partially Pickedup'
	...
	//ELSE update product
			...
			$update2 = $conn->query("UPDATE orders SET PickedupQty=PickedupQty+'$row[OrderedQty]', PickedupStatus='Pickedup'
				WHERE ProductID='$row[ProductID]' AND OrdersID='$ordersid'");	
	}
	...
	if($row['CategoryReturned'] == 1) {
		...
		if($tankscheck->num_rows > 0) {
			...
			$tank_insert = $conn->query("INSERT INTO tanks(OrdersID, ProductID, Qty, Status, ReturnedQty)
					VALUES('$ordersid', '$row[ProductID]', '$row[OrderedQty]', 'Pending', '0')");
		}
	}
...
header('location: pickup_prepare2.php?ordersid='.$ordersid. '');
